...
export function ArenaPage({ demo = false, local = false, official = false }: { demo?: boolean; local?: boolean; official?: boolean }) {
  ...
  const tacticMovements = useMemo(() => game.observation?.tactics?.flatMap((diagnostics) => diagnostics.movement) ?? [], [game.observation])
  ...
  return <div className="grid h-dvh min-h-[560px] grid-cols-1 overflow-hidden lg:grid-cols-[260px_1fr]">
    ...
    <section className="relative min-h-0 overflow-hidden">
      {!respawning && local && game.localSession?.mode === 'step' && game.tick && game.liveTick
        ? <LocalStepControl tick={game.tick} liveTick={game.liveTick} phase={game.phase} status={game.localStatus} history={game.localHistory} replay={game.replay} localView={game.localView} observationPending={game.observationPending} godSnapshot={game.godSnapshot} onAdvance={game.advance} onReplay={game.showReplay} onReturnLive={game.returnLive} onBranch={game.branchFromReplay} onObservation={game.setLocalObservation} onLoadGodDiagnostics={game.loadGodDiagnostics} onHumanFullVision={game.setHumanFullVision} onAddParticipant={game.addLocalParticipant} onSetTickLabel={game.setTickLabel} />
