...
export function LocalStepControl({
  ...
  return <div className="pointer-events-none absolute right-3 top-3 z-30 flex w-[min(34rem,calc(100%_-_1.5rem))] flex-col items-end">
    <section className="panel pointer-events-auto grid h-12 w-full grid-cols-[4.75rem_minmax(0,1fr)_2.25rem_2.25rem_2.25rem_2.25rem_2.25rem] items-center gap-1.5 rounded-gold p-1.5 sm:grid-cols-[5.75rem_minmax(7rem,1fr)_7.25rem_2.25rem_2.25rem_2.25rem_2.25rem]" aria-label={t('game.localStep')}>
      ...
      <button type="button" onClick={() => openTab('view')} aria-busy={observationPending} className="focus-ring grid h-9 min-w-0 grid-cols-[.75rem_minmax(0,1fr)_.75rem] items-center gap-1.5 rounded-gold border border-white/10 bg-white/[.025] px-2 text-[10px] text-zinc-300 hover:bg-white/[.06]" aria-label={t('game.controlView')} title={viewLabel}>
      ...
      {runMode === 'batch' ? <button type="button" onClick={() => openTab('advance')} aria-label={t('game.batchTickProgress', { count: remainingTicks })} className="focus-ring flex h-9 min-w-0 items-center justify-center gap-1 overflow-hidden rounded-gold border border-cyan-signal/25 bg-cyan-signal/10 px-1.5 font-mono text-[9px] tabular-nums text-cyan-signal" aria-live="polite">
        <FastForward size={12} className="shrink-0" /><span className="hidden min-w-0 truncate sm:block">{remainingTicks}</span>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-4">
        {activeTab === 'advance' && <div>
          ...
          {!replay && <div className="mt-4 grid gap-3">
            ...
            <div className="min-h-8" aria-live="polite"><p aria-hidden={!runnerActive} className={`rounded-gold bg-cyan-signal/[.06] px-3 py-2 font-mono text-[9px] text-cyan-signal transition-opacity ${runnerActive ? 'opacity-100' : 'pointer-events-none opacity-0'}`}>{runMode === 'batch' ? t('game.batchTickProgress', { count: remainingTicks }) : t('game.autoTickRunning', { seconds: autoTickSecondsText })}</p></div>
          </div>}
          <div className="mt-4 min-h-[3.25rem] border-t border-white/[.07] pt-3" aria-live="polite">
